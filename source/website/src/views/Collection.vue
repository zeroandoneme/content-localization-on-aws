  ...
  <div>
    <div class="headerTextBackground">
      ...
      <b-container fluid>
        ...
        <b-alert v-model="showDataplaneAlert" variant="danger" dismissible>
          Dataplane Error. Please check browser and dataplane logs.
        </b-alert>
        <b-alert v-model="showDeletedAlert" variant="success" dismissible fade>
        ...
        <b-row class="my-1" align-v="center" align-h="center">
          <b-col sm="5">
            <input v-model="user_defined_query" type="text" placeholder="Search Collection..."
              @keyup.enter="searchCollection">
          </b-col>
          <b-col sm="1">
            <b-button size="lg" @click="searchCollection">
              Search
            </b-button>
          </b-col>
        </b-row>
        ...
        <b-col>
          <div>
            <div class="column">
              <b-row class="my-1">
                <b-col>
                  <b-table striped hover fixed responsive show-empty :fields="fields" :items="asset_list"
                    :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :current-page="currentPage" :per-page="perPage">
                    <template #cell(Thumbnail)="data">
                      <VideoThumbnail :thumbnail-i-d="data.item.thumbnailID" :signed-url="data.item.signedUrl" />
                    </template>
                    ...
                    <template #cell(status)="data">
                      ...
                      <a v-if="data.item.status !== 'Queued'" href=""
                        @click.stop.prevent="openWindow(data.item.state_machine_console_link)">{{ data.item.status }}</a>
                      <div v-if="data.item.status === 'Queued'">
                        {{ data.item.status }}
                      </div>
                    </template>
                    <template #cell(Actions)="data">
                      <b-link :href="(`/analysis/${data.item.asset_id}`)">
                        Analyze
                      </b-link>
                      <br>
                      <b-link class="text-danger" @click="deleteAsset(`${data.item.asset_id}`)">
                        Delete
                      </b-link>
                    </template>
                  </b-table>
                  <div v-if="noAssets">
                    <p>
                      Looks like no assets have been uploaded! Try uploading <a href="upload"
