...
<template>
  <div>
    <div v-if="noTranscript === true">No transcript found for this asset</div>
    <div v-if="isBusy">
    ...
    <div v-else>
      {{ transcript }}
      <br />
      <b-button v-if="transcript.length > 0" id="downloadTranscript" size="sm" class="mb-2"
...
export default {
  ...
  methods: {
    async fetchAssetData() {
      let query =
        "AssetId:" + this.$route.params.asset_id + " _index:mievideotranscript";
      let apiName = "search";
      let path = "/_search";
      let apiParams = {
